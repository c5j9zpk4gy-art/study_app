...
def generate_ai_report(basic_info, survey_answers):
    ...
    [작성 요구 조건]
    1. 요약하지 말고 세밀하고 길게 서술할 것, 5000자 이상의 분량으로 사용자가 분석력이 좋다고 느낄정도.
    2. 학생이 입력한 설문 대답을 직접 인용하면서, 그 습관이 성적에 미치는 영향과 해결책을 구체적으로 설명할 것, 여기서 해결책을 상세하고 구체적인 방법으로 예시를 들어 쉽게 풀어쓸 것.
    3. D-{basic_info.get('days')}일 간의 일차별 스케줄을 구체적인 분량, 교재명과 함께 정밀하게 직접 짜줄 것.
    4. 다음 목차 구조를 반드시 준수할 것.
       - 1. 현재 학업 위치를 백분위를 언급하여 객관적으로 설명하고 목표 위치까지의 도달 방법을 설명하며 구체적인 컨설팅
       - 2. 설문 응답 기반 1:1 약점 패턴 분석 및 행동 교정 처방
       - 3. 학교 출제 난이도 맞춤형 주교재/부교재 및 회독 전략
       - 4. AI 맞춤 D-{basic_info.get('days')} 초밀착 일차별 학습 스케줄
       - 5. 실전 시험장 타임어택 및 실수 방지 페이스메이킹 전략을 세워 이를 대비하는 방법을 설명할 것.
    """
...
def render_survey_custom():
    ...
    questions = [
        ...
        ("질문 7. 고난도 문항을 접했을 때 상태는?", [ "접근조차 하지 못하고 포기한다" "아이디어는 떠오르나 막힘", "시간 충분하면 풂"])
    ]
