...
# 3. AI 리포트 생성 함수 (5등급제 반영 & 규격화 템플릿 적용)
def generate_ai_report(basic_info, survey_answers):
    ...
    try:
        ...
        [⚠️ 중요: 내신 등급 체계 기준]
        이 평가 시스템은 기존 9등급제가 아닌 **'고교학점제 5등급제'** 기준입니다. 반드시 아래 5등급제 산출 기준을 엄격히 적용하여 정밀 진단하세요.
        - **1등급**: 상위 10% 이내
        - **2등급**: 상위 10% 초과 ~ 34% 이내
        - **3등급**: 상위 34% 초과 ~ 66% 이내
        - **4등급**: 상위 66% 초과 ~ 90% 이내
        ...
        - 현재 성적: 전교 {basic_info.get('student_rank')}등 / 전체 {basic_info.get('total_students')}명 (상위 {basic_info.get('pct')}%, 5등급제 기준 현재 등급: {basic_info.get('calc_grade')}등급)
...
def render_basic_input():
    ...
    with st.form("basic_info_form"):
        ...
        if st.form_submit_button("다음: 심층 학습 습관 설문 작성하기", use_container_width=True):
            ...
            # 5등급제 산출 기준 (1등급 10%, 2등급 34%, 3등급 66%, 4등급 90%, 5등급 100%)
...
def render_survey_custom():
    ...
    questions = [
        ...
        ("Q6. 고난도 킬러 문항(상위 10% 1등급 변별력 문제)을 접근할 때의 상태는 어떠한가요?", [
...
def render_result():
    ...
    with st.spinner("🎓 대치동 AI 컨설턴트가 5등급제 기준으로 정밀 리포트를 생성 중입니다..."):
