...
def render_result():
    ...
    if days < 20 or grade >= 4:
        # 기간이 짧거나 기본기가 부족한 경우: 2권으로 부담 완화
    ...
    # 4. 설문 답변 기반 동적 분석 및 솔루션 생성 (20가지 이상 개별 조건 반응)
    q4_ans = survey.get('q_4', '')  # 실수 빈도
    q6_ans = survey.get('q_6', '')  # 시간 배분
    q7_ans = survey.get('q_7', '')  # 킬러 태도
    q13_ans = survey.get('q_13', '') # 해설지 의존도
    ...
    # 6. 세부 일차별 학습 스케줄 (추천 교재 명칭 100% 반영)
    st.markdown("""
