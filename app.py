...
# Streamlit Secrets에서 Gemini API 키 자동 로드
GEMINI_API_KEY = st.secrets.get("GEMINI_API_KEY", "")

# 3. 5,000자 이상 초장문 AI 컨설팅 리포트 생성 함수 (Gemini API)
def generate_ai_report(basic_info, survey_answers):
    ...
    [작성 요구 조건 - 필수 5,000자 이상]
    1. 요약하지 말고 매우 세밀하고 길게 서술할 것.
    ...
    4. 다음 목차 구조를 반드시 준수할 것:
       ...
       - 4. AI 맞춤 D-{basic_info.get('days')} 초밀착 일차별 학습 스케줄 (1일차부터 D-DAY까지 전 일차 상세 플랜)
    ...
    if not api_key:
        return "⚠️ **서버에 Gemini API 키가 설정되지 않았습니다.**\nStreamlit Cloud Secrets에 GEMINI_API_KEY를 등록해주세요."
...
def render_home():
    ...
    st.write("Gemini AI 기반 5,000자 이상 초밀착 맞춤 리포트 및 랩타임 분석기")
    st.divider()
...
def render_survey_custom():
    ...
    questions = [
        ...
        ("질문 3. 시험에서 가장 자주 발생하는 오답과 감점의 주요 원인은 무엇인가요?", 
         ["계산 실수 및 중간 과정의 기호 오용", "조건 해석 오인으로 인한 초기 접근 오류", "시간 부족으로 인한 뒤쪽 고난도/서술형 미풀이", "개념 적용 자체를 못함"]),
