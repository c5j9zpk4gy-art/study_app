...
# -----------------------------------------------------------------------------
# 1. 페이지 설정 및 네이티브 앱 UI CSS (다크모드 완벽 대응 강제 지정)
# -----------------------------------------------------------------------------
st.set_page_config(page_title="고등 수학 내신 솔루션", layout="centered")

st.markdown("""
    <style>
    /* 다크모드 대응: 전체 앱 배경 및 글자색 강제 고정 */
    .stApp {
...
def render_basic_input():
    ...
    with st.form("basic_info_form"):
        ...
        • <b>Level 4 [최상위 학군지]</b>: 강남3구, 대치, 서초, 목동, 중계, 분당, 수성구 등.<br>
        • <b>Level 3 [주요 거점 학군지]</b>: 천안, 청주, 전주, 창원, 수원, 해운대 등 주요 거점.<br>
        • <b>Level 2 [일반계 고교]</b>: 전국 평준화/비평준화 일반계 고교 평균 수준.<br>
        • <b>Level 1 [기초 개념 중심]</b>: 소규모 고교/기초 학력 위주 시험 학교.
        """, unsafe_allow_html=True)
...
def render_result():
    ...
    # 2. 기간 및 학생 위치별 현실적 교재 권수 선정 (2권 ~ 3권)
    days = info['days']
    ...
    if days < 20 or grade >= 4:
        if "Level 4" in reg or "Level 3" in reg:
    ...
    # 다크모드 글자색 보완: code 태그 대신 직접 커스텀 뱃지 클래스 적용
    st.markdown('<div class="app-card">', unsafe_allow_html=True)
    ...
    # 3. 학군지별 동적 분석문 생성
    if "Level 4" in reg:
        region_desc = "선택하신 <b>Level 4 [최상위 학군지]</b>는 변별력을 위해 강남/대치권 족보 및 수능 킬러 변형 문항이 대거 출제됩니다. 시중 단순 유형서만으로 고득점이 불가능하며, 조건 제시형 고난도 문제의 구조적 해석 능력이 핵심 평가 요소입니다."
    elif "Level 3" in reg:
        region_desc = "선택하신 <b>Level 3 [주요 거점 학군지]</b>는 대표 유형을 넘어서는 준심화 변형 문항이 3~5문항 출제되어 등급을 가릅니다. 개념의 단순 적용을 넘어 유형간 융합 문항 풀이 연습이 필수적입니다."
    elif "Level 2" in reg:
        region_desc = "선택하신 <b>Level 2 [일반계 고교]</b>는 교과서 및 시중 대표 유형서(쎈/RPM) 출제 비중이 매우 높습니다. 킬러 문항에 연연하기보다 실수 없이 대표 유형을 완벽하게 소화하는 정답률 관리가 최고 효율을 냅니다."
    else:
        region_desc = "선택하신 <b>Level 1 [기초 개념 중심]</b>은 연산 능력과 기본적인 개념 정의 이해를 묻는 출제가 주를 이룹니다. 복잡한 심화 교재보다는 기본 예제와 대표 문제를 빠르게 반복하여 실수를 줄이는 것이 핵심입니다."
    ...
    if "시간이 부족" in q6_ans:
        if "2~3문항 이상 실수" in q4_ans:
            survey_solutions.append("<b>[시간 부족 + 높은 실수율]</b> 시험 풀이 속도가 둔해 조급해지면서 계산 실수가 연달아 발생하는 전형적인 '시간 압박형 실수' 패턴입니다. 이는 개념 문제가 습관적으로 안 풀리고 매번 생각해서 풀기 때문입니다. 대표 유형의 자동화 풀이가 우선되어야 합니다.")
        else:
            survey_solutions.append("<b>[시간 부족 + 실수 적음]</b> 정확도는 높으나 풀이 속도가 느려 점수가 정체되는 상태입니다. 시험 시 1문항당 최대 2분을 넘기지 않고 별표 치고 넘어가는 '타임 어택 스킵 전략'과 풀이 단순화 훈련이 시급합니다.")
    elif "10분 이상 남아서" in q6_ans:
        if "2~3문항 이상 실수" in q4_ans:
            survey_solutions.append("<b>[시간 충분 + 높은 실수율]</b> 문제를 빠르게 푸는 습관은 좋으나, 문제를 대충 읽거나 암산 과정에서 허점이 발생하는 '시야 협소형 실수'가 심각합니다. 풀이 과정 작성 시 줄글 구분을 명확히 하고, 검산 시에는 정풀이 역방향 검산을 도입해야 합니다.")
        else:
            survey_solutions.append("<b>[시간 충분 + 실수 적음]</b> 기본기가 매우 단단하며 상위권으로 도약할 최적의 조건을 갖췄습니다. 이제는 준심화 및 최심화 킬러 문항에 소요 시간을 쏟아부어 상위 1% 백분위를 굳히는 전략을 써야 합니다.")
    else:
        survey_solutions.append("<b>[시간 배분 보통]</b> 시험 시간 운용은 안정적이나, 고득점 문항 진입 시 소요되는 시간이 길어져 안전성이 떨어집니다. 구간별 풀이 시간을 기계적으로 체크하는 습관이 필요합니다.")

    if "1~2분 고민 후" in q13_ans:
        survey_solutions.append("<b>[해설지 즉시 확인 습관]</b> 막힐 때마다 해설지를 바로 보면 '아는 느낌'만 들 뿐, 시험장에서 스스로 첫 아이디어를 떠올리는 뇌 근육이 자라지 않습니다. 모르는 문제는 최소 10분간 손으로 그래프나 식을 시도해 본 뒤 해설을 한 줄만 힌트로 보는 방식으로 바꾸셔야 합니다.")
    elif "끝까지 혼자 고민" in q13_ans:
        survey_solutions.append("<b>[자기주도 고민 우수]</b> 스스로 고민하는 집념이 훌륭하여 심화 사고력 완성 가능성이 매우 높습니다. 다만 시험 직전에는 제한 시간이 있으므로 고민 시간을 최대 15분으로 제한하고 효율성을 챙기는 조정이 필요합니다.")
    else:
        survey_solutions.append("<b>[적정 고민 시간 유지]</b> 10분 내외의 타당한 고민 시간을 갖고 있어 학습 밸런스가 양호합니다. 해설지 수록 풀이 외에 제2의 풀이법(기하적 해석 등)이 있는지 비교하는 습관을 더해보세요.")

    if "포기함" in q7_ans:
        survey_solutions.append("<b>[킬러 문항 진입 장벽]</b> 고난도 문항에 대한 심리적 두려움이 큽니다. 처음부터 끝까지 혼자 푸는 것을 목표로 삼지 말고, 킬러 문항의 '조건 1개만 해독하기'부터 시작하여 진입 장벽을 낮춰야 합니다.")
    elif "해결 가능" in q7_ans:
        survey_solutions.append("<b>[킬러 문항 소화 가능]</b> 고난도 문항 해결력이 뛰어나므로, 오답 노트 작성 시 문제의 핵심 아이디어를 1문장으로 요약하는 '키포인트 정리'에 집중하면 완벽해집니다.")
    ...
    학생분의 현재 위치는 전체 {info['total_students']}명 중 전교 <b>{info['student_rank']}위</b>로, 상위 <b>{info['pct']}%</b>에 위치하고 있습니다. 5등급제 기준 현재 <b>{info['calc_grade']}등급</b>이며, 목표인 <b>{info['target_grade']}등급</b> 달성을 위해 남아있는 백분위 격차를 정밀하게 메워야 합니다.
    </div>
    ...
    <div class="report-paragraph">
    <b>3. 20문항 설문 기반 동적 맞춤 약점 극복 솔루션</b><br>
    {survey_full_text}
    </div>

    <div class="report-paragraph">
    <b>4. 남은 {info['days']}일 학습 방향성 및 실전 전략</b><br>
    남은 기간 동안 과도하게 많은 문제집을 탐내기보다, 선정된 <b>{len(rec_books)}권의 핵심 교재({', '.join(rec_books)})</b>를 완벽히 소화하는 것이 핵심입니다. 시험 7일 전부터는 추가 문제 풀이를 멈추고 틀린 문제 누적 회독과 학교 기출 타임어택으로 마무리를 지어야 합니다.
    </div>
    """, unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
...
def render_stopwatch():
    ...
    if st.session_state.sw_running:
        st.session_state.sw_elapsed_time = time.time() - st.session_state.sw_start_time
        timer_placeholder.markdown(f"<h1 style='text-align: center; color: #2D65F2 !important; font-size: 44px; margin: 24px 0;'>{format_time(st.session_state.sw_elapsed_time)}</h1>", unsafe_allow_html=True)
        time.sleep(0.1)
        st.rerun()
    else:
        timer_placeholder.markdown(f"<h1 style='text-align: center; color: #8B95A1 !important; font-size: 44px; margin: 24px 0;'>{format_time(st.session_state.sw_elapsed_time)}</h1>", unsafe_allow_html=True)
