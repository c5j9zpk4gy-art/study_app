...
CURRICULUM_DETAILED_DB = {
    "공통수학1": [
        ...
        ("항등식과 미정계수법", "항등식의 성질, 수치대입법 및 계수비교법실전 적용"),
    ...
    "공통수학2": [
        ...
        ("함수의 뜻과 합성함수", "함수의 정의, 일대일대응, 합성함수 및 역함수 해석"),
...
def render_result():
    ...
    # 4. 소단원별 초밀착 일차별 스케줄 (병행 및 분량 반영)
    st.markdown("""
        <div style="margin-top: 24px; margin-bottom: 12px;">
            <div class="app-header" style="font-size: 18px;">초밀착 세부 일차별 학습 스케줄</div>
            <div class="app-subtext">단원별 소단원을 2~3일 단위로 세분화하여 교재 병행 학습을 수행하는 실행 계획입니다.</div>
        </div>
    """, unsafe_allow_html=True)

    full_units = CURRICULUM_DETAILED_DB.get(info['subject'], [("전체 단원", "개념 및 유형 학습")])
    ...
    days_per_subunit = max(2, (total_days - 5) // num_sub_units)
